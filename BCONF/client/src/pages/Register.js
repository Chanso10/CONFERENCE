...
const Register = ({ setUser }) => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const res = await axios.post(`${process.env.REACT_APP_API_URL}/api/auth/register`, { ...form, isAttendee });
            ...
            <form className="panel auth-card auth-form" onSubmit={handleSubmit}>
                ...
                <div className="form-grid">
                    ...
                    <label className="field" style={{display: 'flex', alignItems: 'center'}}>
                        <input type="checkbox" checked={isAttendee} onChange={(e) => setIsAttendee(e.target.checked)} />
                        <span> I will be attending the conference</span>
                    </label>
                    {isAttendee && (
                        <>
                            <label className="field">
